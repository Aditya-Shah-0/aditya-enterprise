import React from "react";
import { useNavigate } from "react-router-dom";
import { Search, Plus, Eye, Trash2, ScrollText, FileSignature } from "lucide-react";

const RecentQuotationsTable = ({
  filteredQuotations,
  searchQuery,
  setSearchQuery,
  onStatusChange,
  onDeleteQuotation,
  onConvertToInvoice
}) => {
  const navigate = useNavigate();

  const getStatusStyle = (status) => {
    switch (status) {
      case "Accepted":
        return "bg-green-50 text-green-700 border border-green-200";
      case "Sent":
        return "bg-blue-50 text-blue-700 border border-blue-200";
      case "Declined":
        return "bg-red-50 text-red-700 border border-red-200";
      case "Invoiced":
        return "bg-purple-50 text-purple-700 border border-purple-200";
      case "Draft":
      default:
        return "bg-gray-50 text-gray-700 border border-gray-200";
    }
  };

  return (
    <>
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between px-6 py-4 text-black bg-white rounded-xl border border-gray-200 shadow-sm mb-6">
        <div className="relative w-full sm:w-80">
          <Search className="absolute left-3 top-2.5 size-4 text-gray-400" />
          <input
            type="text"
            placeholder="Search by client, quote number, date..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full pl-9 pr-4 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-gray-50/50 placeholder-gray-400 transition-all"
          />
        </div>
        <button
          onClick={() => navigate("/app/quotation/create")}
          className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-medium text-sm px-4 py-2.5 rounded-lg shadow-sm hover:shadow transition-all cursor-pointer w-full sm:w-auto justify-center"
        >
          <Plus className="size-4" /> Create Quotation
        </button>
      </div>

      <div className="lg:col-span-2 bg-white p-6 rounded-xl shadow-sm border border-gray-200">
        <h3 className="text-lg font-bold text-gray-900 mb-4">All Estimations / Quotations</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm border-collapse">
            <thead className="bg-gray-50/75 uppercase text-[10px] tracking-wider font-semibold text-gray-500 border-b border-gray-200">
              <tr>
                <th className="px-4 py-3 text-left">Date</th>
                <th className="px-4 py-3 text-left">Quote No</th>
                <th className="px-4 py-3 text-left">Party Name</th>
                <th className="px-4 py-3 text-left">Total Value</th>
                <th className="px-4 py-3 text-left">Status</th>
                <th className="px-4 py-3 text-center">Actions</th>
              </tr>
            </thead>
            {filteredQuotations?.length > 0 ? (
              <tbody className="divide-y divide-gray-100">
                {filteredQuotations.map((quote, index) => {
                  return (
                    <tr
                      key={quote._id || index}
                      className="hover:bg-gray-50/50 transition-colors"
                    >
                      <td className="px-4 py-3.5 text-gray-700 font-medium">
                        {quote.date ? quote.date.split("T")[0] : ""}
                      </td>
                      <td className="px-4 py-3.5 text-gray-900 font-semibold">{quote.quotationNo}</td>
                      <td className="px-4 py-3.5 text-gray-900 font-bold">{quote.partyName}</td>
                      <td className="px-4 py-3.5 text-gray-900 font-bold">
                        ₹ {(quote.grandTotal || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </td>
                      <td className="px-4 py-3.5">
                        <select
                          value={quote.status}
                          onChange={(e) => onStatusChange(quote._id, e.target.value)}
                          className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold cursor-pointer outline-none ${getStatusStyle(quote.status)}`}
                        >
                          <option value="Draft">Draft</option>
                          <option value="Sent">Sent</option>
                          <option value="Accepted">Accepted</option>
                          <option value="Declined">Declined</option>
                          <option value="Invoiced">Invoiced</option>
                        </select>
                      </td>
                      <td className="px-4 py-3.5">
                        <div className="flex gap-2 items-center justify-center">
                          <button
                            onClick={() => navigate(`/app/quotation/view/${quote._id}`)}
                            className="p-1.5 text-blue-600 hover:text-blue-700 hover:bg-blue-50 rounded-lg transition-colors cursor-pointer"
                            title="View Estimation"
                          >
                            <Eye className="size-5" />
                          </button>
                          
                          {quote.status !== "Invoiced" && (
                            <button
                              onClick={() => onConvertToInvoice(quote)}
                              className="p-1.5 text-purple-650 hover:text-purple-800 hover:bg-purple-50 rounded-lg transition-colors cursor-pointer"
                              title="Convert to Sale Invoice"
                            >
                              <ScrollText className="size-5" />
                            </button>
                          )}

                          <button
                            onClick={() => onDeleteQuotation(quote._id)}
                            className="p-1.5 text-red-600 hover:text-red-700 hover:bg-red-50 rounded-lg transition-colors cursor-pointer"
                            title="Delete"
                          >
                            <Trash2 className="size-5" />
                          </button>
                        </div>
                      </td>
                    </tr>
                  );
                })}
              </tbody>
            ) : (
              <tbody>
                <tr>
                  <td colSpan={6}>
                    <div className="flex flex-col items-center justify-center py-10 text-center w-full">
                      <div className="bg-gray-100 p-2 rounded-full mb-3">
                        <FileSignature className="size-24 text-gray-400" />
                      </div>
                      <p className="text-lg font-semibold text-gray-700">No estimations or quotations made yet!</p>
                      <p className="text-sm text-gray-500 mt-1">
                        Create your first quote to present estimations to clients.
                      </p>
                    </div>
                  </td>
                </tr>
              </tbody>
            )}
          </table>
        </div>
      </div>
    </>
  );
};

export default RecentQuotationsTable;
