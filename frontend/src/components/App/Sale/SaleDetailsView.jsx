...
import { 
  ArrowLeft, Edit3, Printer, BanknoteArrowUp, Calendar, MapPin, 
...
export const SaleDetailsView = () => {
  ...
  const sale = transaction?.transactions?.find((item) => item._id === id);
    ...
    <div className="min-h-screen bg-gray-50/50 p-4 sm:p-6 lg:p-8 text-black">
      ...
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 pb-4 border-b border-gray-200">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-xl font-bold text-gray-900">Invoice: {sale.invoiceNo}</h2>
              <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold border ${
                sale.isPaid 
                  ? "bg-emerald-50 text-emerald-700 border-emerald-200"
                  : "bg-red-50 text-red-700 border-red-200"
              }`}>
                <span className={`size-1.5 rounded-full ${sale.isPaid ? "bg-emerald-500" : "bg-red-500"}`} />
                {sale.isPaid ? "Paid" : "Unpaid / Outstanding"}
              </span>
            </div>
            <p className="text-xs text-gray-500 mt-0.5">Created on {sale.date ? sale.date.split("T")[0] : ""}</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          {!sale.isPaid && (
            ...
            >
              <BanknoteArrowUp size={16} /> Record Payment
            </button>
          )}
          ...
          <button
            onClick={() => navigate(`/app/sale/invoiceview/${sale._id}`)}
            className="flex items-center gap-2 bg-white hover:bg-gray-100 text-gray-700 px-4 py-2 border border-gray-250 rounded-lg text-sm font-semibold shadow-xs transition-all cursor-pointer"
          >
            <Printer size={16} /> Print View
          </button>

          <button
            onClick={() => navigate(`/app/sale/edit/${sale._id}`)}
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-xs grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <div className="space-y-2 text-sm">
                <p className="font-bold text-gray-900 text-base">{sale.partyName}</p>
                
                {sale.partyPhone && (
                  <p className="text-gray-600 flex items-center gap-2">
                    <Phone size={14} className="text-gray-400" />
                    <span>{sale.partyPhone}</span>
                    <button 
                      onClick={() => handleCopy(sale.partyPhone, "Phone")}
            ...
            <div>
              <h3 className="text-sm font-bold uppercase tracking-wider text-gray-400 mb-3 flex items-center gap-1.5">
                <FileText size={16} className="text-gray-400" /> Billing Settings
              </h3>
              <div className="space-y-2 text-sm text-gray-600">
                <div className="flex justify-between py-1 border-b border-gray-50">
                  <span className="font-medium">Supply State:</span>
                  <span className="font-semibold text-gray-900">{sale.stateOfSupply || "N/A"}</span>
                </div>
                <div className="flex justify-between py-1 border-b border-gray-50">
                  <span className="font-medium">Term:</span>
                  <span className="font-semibold text-gray-900">{sale.term || "N/A"}</span>
                </div>
                <div className="flex justify-between py-1 border-b border-gray-50">
                  <span className="font-medium">Due Date:</span>
                  <span className="font-semibold text-red-600 flex items-center gap-1">
                    <Calendar size={13} /> {sale.dueDate ? sale.dueDate.split("T")[0] : "N/A"}
                  </span>
                </div>
                <div className="flex justify-between py-1">
                  <span className="font-medium">Initial Mode:</span>
                  <span className="font-semibold text-gray-900 bg-gray-100 px-2 py-0.5 rounded text-xs">{sale.paymentMode || "Cash"}</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm border-collapse">
                ...
                <tbody className="divide-y divide-gray-100 font-medium">
                  {sale.particulars && sale.particulars.length > 0 ? (
                    sale.particulars.map((item, idx) => (
                      <tr key={idx} className="hover:bg-gray-50/50">
                        ...
                        <td className="px-3 py-3 text-right text-gray-700">{(item.price || 0).toFixed(2)}</td>
                    ...
                    <tr>
                      <td colSpan="5" className="px-3 py-6 text-center text-gray-400">
                        No particulars listed on this invoice.
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          <div className="bg-white border border-gray-250 rounded-xl p-5 shadow-xs text-sm">
            ...
            <div className="space-y-2.5 border-b border-gray-100 pb-3">
              <div className="flex justify-between text-gray-600">
                <span>Subtotal:</span>
                <span className="font-semibold text-gray-900">₹ {(sale.subTotal || 0).toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-gray-600">
                <span>Discount ({sale.discountPercentage || 0}%):</span>
                <span className="font-semibold text-gray-900">
                  - ₹ {(((sale.subTotal || 0) * (sale.discountPercentage || 0)) / 100).toFixed(2)}
                </span>
              </div>
              <div className="flex justify-between text-gray-600">
                <span>GST ({sale.taxPercentage || 0}%):</span>
                <span className="font-semibold text-gray-900">
                  + ₹ {(((sale.subTotal || 0) * (sale.taxPercentage || 0)) / 100).toFixed(2)}
                </span>
              </div>
            </div>
            ...
            <div className="space-y-2.5 py-3 border-b border-gray-100 text-base font-bold">
              <div className="flex justify-between text-gray-900">
                ...
                <span className="text-blue-650">₹ {(sale.grandTotal || 0).toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-gray-600 text-sm font-medium">
                <span>Paid Amount:</span>
                <span className="text-emerald-600 font-bold">₹ {(sale.paidAmount || 0).toFixed(2)}</span>
              </div>
            </div>

            <div className="flex justify-between items-center pt-3 font-extrabold text-lg">
              <span className="text-gray-750">Balance Due:</span>
              <span className={sale.balance > 0 ? "text-red-600" : "text-emerald-600"}>
            ...
            <div className="mt-4 bg-gray-50 border border-gray-200 rounded-lg p-3 text-[11px] leading-relaxed text-gray-500 italic">
              ...
              {amountToWords(sale.grandTotal)} Only
            </div>
          </div>
      ...
      {isPaymentModalOpen && (
        <PaymentModal
          ...
          onSave={async (data) => {
            await transactionService.updateTransaction(sale._id, data);
...
const LoaderComponent = () => (
  <div className="flex flex-col items-center justify-center p-8">
    ...
    <span className="text-sm font-semibold">Loading Invoice details...</span>
  </div>
);
