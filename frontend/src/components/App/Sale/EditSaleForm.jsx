...
const EditSaleForm = () => {
  ...
  // Pre-populate form when invoice loads
  useEffect(() => {
    if (sale) {
      reset({
        ...
        stateOfSupply: sale.stateOfSupply || "Assam",
        particulars: sale.particulars || [],
        discountPercentage: sale.discountPercentage || 0,
        taxPercentage: sale.taxPercentage || 18,
        paymentMode: sale.paymentMode || "Cash",
        paidAmount: sale.paidAmount || 0,
        term: sale.term || "Net 3",
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await transactionService.modifyTransaction(id, finalPayload);
      await refreshTransactions();

      toast.success("Invoice updated successfully");
      navigate(`/app/sale/view/${id}`);
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="h-screen flex flex-col w-full rounded-lg text-black bg-gray-50">
      ...
      <div className="bg-white px-6 py-3 border-b flex items-center gap-3 shrink-0 rounded-t-lg">
        <button
          type="button"
          onClick={() => navigate(`/app/sale/view/${id}`)}
          className="text-gray-600 hover:text-red-500 transition-colors cursor-pointer"
        >
          <ArrowBigLeft size={30} />
        </button>
        <h2 className="text-xl font-bold text-gray-800">Edit Invoice: {sale?.invoiceNo || ""}</h2>
      </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    {...register("invoiceNo")}
                    readOnly
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none bg-gray-100 text-gray-500 font-semibold cursor-not-allowed"
                    title="Invoice numbers are read-only locked once created."
                  />
                  {errors.invoiceNo && (
            ...
            <div className="border border-gray-400 rounded-lg p-5 bg-white shadow-sm">
              ...
              <div className="mt-6 flex gap-3">
                <button
                  ...
                  title="Press Ctrl+S or Alt+S to save changes"
                >
                  <Save /> Save Changes <span className="text-xs bg-blue-700 px-1.5 py-0.5 rounded opacity-80 font-normal ml-1">Ctrl + S</span>
                </button>
              </div>
            </div>
          </div>
